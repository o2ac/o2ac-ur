...
def main():
    ...
    # approach_pose = conversions.to_pose_stamped("cooking_board_surface", [0.050, -0.033, 0.201, 3.064, 0.094, 2.501])
    # cut_pose = conversions.to_pose_stamped("cooking_board_surface", [0.108, -0.071, 0.145, 3.064, 0.093, 2.501])
    
    # pose = conversions.to_pose_stamped("workspace_center", [-0.338, -0.131, 0.144, -0.415, 0.004, -1.549])
    pose = c.a_bot.move_lin_rel(relative_translation=[0.05,0.0,0], end_effector_link="knife_tool_tip_link", pose_only=True)
    c.a_bot.go_to_pose_goal(pose, end_effector_link="knife_tool_tip_link", move_ptp=True)

    # c.confirm_to_proceed("next?")
    # print("MOVEITTT")
    # pose = c.a_bot.move_lin_rel(relative_translation=[0.05,0,0], end_effector_link="move_group/knife/knife_tip", pose_only=True)
    # c.a_bot.go_to_pose_goal(pose, end_effector_link="move_group/knife/knife_tip", move_ptp=False)

    # c.a_bot.go_to_pose_goal(approach_pose)
    # c.a_bot.go_to_pose_goal(cut_pose)
