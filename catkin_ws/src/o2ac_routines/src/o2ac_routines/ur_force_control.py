...
class URForceController():
    def __init__(self, robot_name, config_file="force_control", tcp_link='robotiq_85_tip_link'):
        self.arm = CompliantController(ft_sensor=True, namespace=robot_name,
                                       joint_names_prefix=robot_name+'_', robot_urdf=robot_name,
                                       robot_urdf_package='o2ac_scene_description', ee_link=tcp_link)
    ...
    def force_control(self, target_force=None, target_positions=None,
        ...
        self.arm.set_wrench_offset(True)  # offset the force sensor
        self.arm.relative_to_ee = relative_to_ee if relative_to_ee is not None else self.arm.relative_to_ee
        self.arm.ee_transform = ee_transform if ee_transform is not None else self.arm.ee_transform

        target_positions = self.arm.end_effector() if target_positions is None else np.array(target_positions)
