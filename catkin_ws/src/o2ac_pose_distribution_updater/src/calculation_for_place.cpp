...
#include "o2ac_pose_distribution_updater/conversions.h"
...
Eigen::Transform<double, 3, Eigen::Isometry>
    gripper_transform; // The gripper transform
...
public:
  ...
  void operator()(const Eigen::Matrix<T, 6, 1> &current_particle,
    ...
    auto result_transform =
        gripper_transform_T.inverse() * Eigen::Translation<T, 3>(final_trans) *
    ...
    Eigen::Quaternion<T> result_rotation(result_transfrom.rotation());
    T roll, pitch, yaw;
...
void place_update_distribution(
    ...
    const Eigen::Transform<double, 3, Eigen::Isometry> _gripper_transform,
