...
class URScriptRelay():
    ...
    def srv_callback(self, req):
        ...
        # Moves forward until a force is detected
            ...
            if not req.max_force:
                req.max_force = 10.0
            if not req.force_direction and not req.direction_vector: 
                req.force_direction = "Z+"
            elif not req.force_direction: # Do not set force_direction if direction_vector is used
                req.force_direction = "using_direction_vector"
            if not len(req.direction_vector) == 3:
                req.direction_vector = [0, 0, 0]
            if not req.forward_speed:
                req.forward_speed = .02
            ...
            ### Function definitions, for reference:
            ### rq_linear_search(direction="Z+",force = 10, speed = 0.004, max_distance = 0.02, direction_vector = [0,0,0], use_base_coords=False )
            
            program_back += "    rq_zero_sensor()\n"
            program_back += "    textmsg(\"Moving forward (linear push).\")\n"
            program_back += "    rq_linear_search(\"" + req.force_direction + "\"," \
                                + str(req.max_force) + "," \
                                + str(req.forward_speed) + "," \
                                + str(req.acceleration) + "," \
                                + str(req.max_approach_distance) + ", [" \
                                + str(req.direction_vector[0]) + ","+ str(req.direction_vector[1]) + ","+ str(req.direction_vector[2]) + "], " \
                                + str(req.use_base_coords) + ")\n"
