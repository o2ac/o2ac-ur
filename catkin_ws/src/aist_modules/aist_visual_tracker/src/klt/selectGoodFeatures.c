...
void
_quicksort(int *pointlist, int n)
{
...
static void
_fillFeaturemap(int x, int y, uchar *featuremap, int mindist, 
		int ncols, int nrows)
{
    int	ix, iy;
...
static void
_enforceMinimumDistance(int *pointlist,		/* featurepoints */
...
#ifdef KLT_USE_QSORT
static int
_comparePoints(const void *a, const void *b)
{
    int v1 = *(((int *) a) + 2);
    int v2 = *(((int *) b) + 2);
...
static float
_minEigenvalue(float gxx, float gxy, float gyy)
{
...
void
_KLTSelectGoodFeatures(KLT_TrackingContext tc, KLT_PixelType *img, 
		       ...
		       selectionMode mode)
{
  ...
  /* Enforce minimum distance between features */
    _enforceMinimumDistance(pointlist, npoints, featurelist,
...
void
KLTSelectGoodFeatures(KLT_TrackingContext tc, KLT_PixelType *img, 
...
void
KLTReplaceLostFeatures(KLT_TrackingContext tc, KLT_PixelType *img, 
		       int ncols, int nrows, KLT_FeatureList fl)
{
