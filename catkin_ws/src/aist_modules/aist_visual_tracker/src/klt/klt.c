...
static void**
_createArray2D(int ncols, int nrows, int nbytes)
{
  char	**tt;
  int	i;
...
KLT_FeatureList
KLTCreateFeatureList(int nFeatures)
{
    KLT_FeatureList	fl;
    KLT_Feature		first;
...
KLT_FeatureHistory
KLTCreateFeatureHistory(int nFrames)
{
...
KLT_FeatureTable
KLTCreateFeatureTable(int nFrames, int nFeatures)
{
    KLT_FeatureTable	ft;
    KLT_Feature		first;
...
void
KLTPrintTrackingContext(KLT_TrackingContext tc)
{
    fprintf(stderr, "\n\nTracking context:\n\n");
...
void
KLTChangeTCPyramid(KLT_TrackingContext tc, int search_range)
{
...
void
KLTUpdateTCBorder(KLT_TrackingContext tc)
{
    float	val;
...
void
KLTFreeTrackingContext(KLT_TrackingContext tc)
{
    if (tc->pyramid_last)        
...
void
KLTFreeFeatureList(KLT_FeatureList fl)
{
  /* for affine mapping */
    int indx;
...
void
KLTFreeFeatureHistory(KLT_FeatureHistory fh)
{
    free(fh);
}

void
KLTFreeFeatureTable(KLT_FeatureTable ft)
{
...
void
KLTStopSequentialMode(KLT_TrackingContext tc)
{
...
int
KLTCountRemainingFeatures(KLT_FeatureList fl)
{
    int count = 0;
    int i;
