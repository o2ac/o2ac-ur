...
from data import WRS_ROOT, AnnotationTransform, WRS2020_Detection, BaseTransform
from data import WRS2020_CLASSES as labelmap
...
parser.add_argument('--wrs_root', default=WRS_ROOT,
...
annopath = os.path.join(args.wrs_root, 'Annotations', 'Far','Image-wise','rgb%s.png.json')
imgpath = os.path.join(args.wrs_root, 'Images', 'Far', 'RGB', 'rgb%s.png')
imgsetpath = os.path.join(args.wrs_root, 'ImageSets', '{:s}.txt')
#YEAR = '2007'
devkit_path = args.wrs_root
...
def parse_rec_wrs(filename):
    """ Parse a WRS json file """
    anno = open( filename )
    ...
    for cls_id in range(len(labelmap)):
        if labelmap[cls_id] in class_id:
            id = class_id.index( labelmap[cls_id] )
            bbox = bboxes[id]
            obj_struct = {}
            key = labelmap[cls_id]
            obj_struct['name'] = key
            ...
            for im_ind, index in enumerate(dataset.ids):
                ...
                for k in range(dets.shape[0]):
                    f.write('{:s} {:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.
                            format(index[0][-8:], index[1], dets[k, -1],
...
def voc_eval(detpath, #検出結果のファイル名
             annopath, #
             imagesetfile, #imagesetの名前
             classname,
             cachedir, #一時データ？のパス．
             ovthresh=0.5,
             use_07_metric=True):
...
# first load gt
    ...
    if not os.path.isfile(cachefile):
        ...
        for i, (scene_id, img_id) in enumerate(zip(scene_ids, img_ids)):
            ...
            recs[imagename] = parse_rec_wrs(annopath % (scene_id, img_id))
            if i % 100 == 0:
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    #まずデータセット全体に対する認識をおこない，BBoxを集める．
...
if __name__ == '__main__':
    ...
    # load data
    dataset = WRS2020_Detection(args.visapp_root, set_type,
